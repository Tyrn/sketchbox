/*!
        ...
        @author   Gavin Lyons
        @brief
                 This file contains the "main" function for  project, a set of
   test sequence to test the HD44780_LCD_PCF8574 arduino  library. 16X02
   display.
        @note
                -# Test 1 :: Hello world
                ...
                -# Test 6 :: Cursor type (4 off) and screen reset test, Cursor
   mode is changed with a reset.
                -# Test 7 :: Text entry mode (4 off) if screen is reset the
   entry mode will be reset to default
...
void cursorTest(void) {
  ...
  myLCD.PCF8574_LCDResetScreen(
      myLCD.LCDCursorTypeOn); // Back to initial state , type 3
  myLCD.PCF8574_LCDGOTO(myLCD.LCDLineNumberTwo, 0);
...
void backLightTest(void) {
  ...
  myLCD.PCF8574_LCDBackLightSet(
      false); // Needs another command/data before it changes Light
  myLCD.PCF8574_LCDGOTO(myLCD.LCDLineNumberTwo, 1);
  myLCD.PCF8574_LCDSendString(teststr4);
  myLCD.print(
      myLCD.PCF8574_LCDBackLightGet()); // read the backlight variable  status
  delay(DISPLAY_DELAY);

  myLCD.PCF8574_LCDBackLightSet(
      true); // Needs another command/data before it changes Light
  myLCD.PCF8574_LCDClearScreen();
}
...
void entryModeTest(void) {
  ...
  myLCD.PCF8574_LCDChangeEntryMode(
      myLCD.LCDEntryModeThree); // Set back to default entry mode
  myLCD.PCF8574_LCDClearScreenCmd();
  delay(DISPLAY_DELAY_1);
}
